...
interface
 uses
   SyncObjs, {$IFDEF Windows} Windows, {$ELSE} LCLType,LCLIntf,dynlibs,LCLProc,{$ENDIF} SysUtils, Classes, Forms, Consts,
   PHPCommon,
...
begin
  try
   ...
   {$IFDEF Windows}
   if Forms.Application.MainFormHandle <> 0 then ShowOwnedPopups(Forms.Application.MainFormHandle, False);
   {$ENDIF}
...
begin
  ...
  //DLLProc := @DLLExitProc;
  {$ifdef WINDOWS}
  Dll_Process_Detach_Hook:=@DLLExitProc;
  {$endif}
end;
