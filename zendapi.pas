...
uses
  {$IFDEF Windows} Windows, {$ELSE} LCLType,dynlibs,libc,{$ENDIF} SysUtils, zendTypes;

const
{$IFDEF PHP_DEBUG}
 {$IFDEF PHP5}
  PHPWin = {$IFDEF Windows} 'php5ts_debug.dll' {$ELSE} 'libphp5ts_debug.so' {$ENDIF};
  {$ELSE}
  PHPWin = {$IFDEF Windows} 'php4ts_debug.dll' {$ELSE} 'libphp4ts_debug.so' {$ENDIF};
  {$ENDIF}
{$ELSE}
 {$IFDEF PHP5}
  PHPWin = {$IFDEF Windows} 'php5ts.dll' {$ELSE} 'libphp5.so' {$ENDIF};
  {$ELSE}
  PHPWin = {$IFDEF Windows} 'php4ts.dll' {$ELSE} 'libphp4ts.so' {$ENDIF};
  {$ENDIF}
{$ENDIF}
...
const
  MSCRT =  {$IFDEF Windows} 'msvcrt.dll' {$else} 'libc.so' {$endif};
  _pipe =  {$IFDEF Windows} '_pipe' {$else} 'pipe' {$endif};
  _close =  {$IFDEF Windows} '_close' {$else} '_close' {$endif};
  _write  =  {$IFDEF Windows} '_write' {$else} '_write' {$endif};
  _setjmp3   =  {$IFDEF Windows} '_setjmp3' {$else} 'setjmp' {$endif};
 _strdup = {$IFDEF Windows} '_strdup' {$else} 'strdup' {$endif};
...
begin
 {$IFDEF QUIET_LOAD}
  Result := false;
 {$ENDIF}
  PHPLib := LoadLibrary(PAnsiChar(DllFileName));
{$IFNDEF QUIET_LOAD}
...
begin
  ...
  {$ELSE}
         memset(@class_container,0,sizeof(Tzend_class_entry));
  {$ENDIF}
