...
uses
  {$IFNDEF FPC} Windows, {$ELSE} LCLType,LCLIntf,dynlibs,libc,{$ENDIF} SysUtils, Classes, Graphics,
  PHPCommon,
  zendTypes,
  PHPTypes,
  zendAPI,
...
function wvsprintfA(Output: PAnsiChar; Format: PAnsiChar; arglist: PAnsiChar): Integer; stdcall; external {$IFDEF Windows} 'user32.dll' {$else} 'libc.so' name 'sprintf' {$endif};
...
begin
  ...
  if assigned(PHPEngine) then
   ...
   //MessageBox(0,pchar(error_type_str),'Error',0);
    ...
    if FUseMapping then
     begin
       {$IFDEF PHP5}
        ...
        {$ELSE}
               FillChar(ZendStream,SizeOf(ZendStream), 0);
        {$ENDIF}
